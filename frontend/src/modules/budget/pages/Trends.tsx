...
import { api } from '../api'
import { fmt, currentMonthStr, prevMonth, nextMonth, monthLabel, monthRange } from '../utils'
...
function MonthNav({ month, onChange }: { month: string; onChange: (m: string) => void }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-sm text-white min-w-32 text-center">{monthLabel(month)}</span>
      <button
...
export default function Trends() {
  ...
  const { data: transactions } = useQuery({
    ...
    queryFn: () => {
      ...
      return api.transactions.list({ start, end })
    },
  })
